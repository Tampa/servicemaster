...
static char *service_format_status(Service *svc)
{
    ...
    case TIMER:
    {
        ...
        if (diff_seconds > 0)
        {
            ...
            if (days > 0)
                ptr += snprintf(ptr, sizeof(buf) - (ptr - buf), "%d days ", days);
            if (hours > 0)
                ptr += snprintf(ptr, sizeof(buf) - (ptr - buf), "%d hours ", hours);
            if (minutes > 0)
                ptr += snprintf(ptr, sizeof(buf) - (ptr - buf), "%d minutes ", minutes);
...
/**
 * Returns the next service in the linked list.
 *
 * @param svc Pointer to the current service
 * @return Pointer to the next service in the list, or NULL if at the end
 */
...
/**
 * Retrieves formatted status information and recent logs for a service.
 *
 * This function:
 * 1. Fetches the current service status via D-Bus
 * 2. Formats the status information into a readable string
 * 3. Appends the last 10 log entries from the service
 *
 * @param bus The bus connection to use
 * @param svc The service to get status information for
 * @return A newly allocated string containing status and logs, or NULL on error.
...
/**
 * Returns a string representation of a service type.
 *
 * @param type The service type to convert to a string
